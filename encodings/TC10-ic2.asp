{s(X,Y); f(X,Y); ex(X,Y); exi(X,Y); alt(X,Y); ret(X,Y); rev(X,Y); i(X,Y); eq(X,Y); dis(X,Y)}=1 :- traj(X), traj(Y), X<Y, #count{R : fact(R,X,Y)} = 0.
eq(X,X) :- traj(X).
...
ex(X,Y) :- fact(ex,X,Y).
exi(X,Y) :- fact(exi,X,Y).
rev(X,Y) :- fact(rev,X,Y).
ret(X,Y) :- fact(ret,X,Y).
